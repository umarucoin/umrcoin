...
*Setup instructions for a Gitian build of UMRcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the UMRcoin
...
- [Building UMRcoin Core](#building-umrcoin-qtcore)
...
Clone the git repositories for umrcoin and Gitian.
...
in the umrcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building UMRcoin Core
----------------

To build UMRcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the umrcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/umrcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/umarucoin/umrcoin
...
For example:
```bash
URL=https://github.com/thrasher-/umrcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit umrcoin=${COMMIT} --url umrcoin=${URL} ../umrcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit umrcoin=${COMMIT} --url umrcoin=${URL} ../umrcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit umrcoin=${COMMIT} --url umrcoin=${URL} ../umrcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the umrcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../umrcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/umrcoin
SIGPATH=/some/root/path/umrcoin-qtdetached-sigs

./bin/gbuild --url umrcoin=${BTCPATH},signature=${SIGPATH} ../umrcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/umrcoin-qtlinux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/umrcoin-qtwin-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/umrcoin-qtosx-build.assert
```
...
[umarucoin/gitian.sigs.umr](https://github.com/umarucoin/gitian.sigs.umr/) repository, or if that's not possible create a pull
