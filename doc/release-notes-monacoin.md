...
UMRcoin Core is extensively tested on multiple operating systems using
...
UMRcoin Core should also work on most other Unix-like systems but is not
...
UMRcoin Core now supports P2SH addresses beginning with P on mainnet and p on testnet.
...
This only affects users that have explicitly enabled UPnP through the GUI setting or through the -upnp option, as since the last UPnP vulnerability (in UMRcoin Core 0.10.4) it has been disabled by default.
...
UMRcoin Core has supported automatically pruning the blockchain since 0.13.2. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/umarucoin/umrcoin/blob/master-0.14/doc/zmq.md) for
...
When UMRcoin Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `umrcoin-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`umrcoin-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/umrcoin-cli -named help command="help"
    src/umrcoin-cli -named getblockhash height=0
    src/umrcoin-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/umrcoin-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of UMRcoin Core. This was added in conjunction with
...
here, originally written for Bitcoin but can also be used for UMRcoin:
